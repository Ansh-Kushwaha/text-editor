...
public class DocumentBenchmarking {
	...
	public static void main(String [] args) {
				...
				numToCheck += increment)
		{
			...
			long basicSTime = System.nanoTime();
			for (int i = 0; i < trials; i++) {
				BasicDocument b = new BasicDocument(text);
				double basicFleschScore = b.getFleschScore();
			}
			
			long basicETime = System.nanoTime();
			double basicTime = (basicETime - basicSTime) / 1000000000.0 / trials;
			
			System.out.format("%.11f\t", basicTime);
			
			long efficientSTime = System.nanoTime();
			for (int i = 0; i < trials; i++) {
				BasicDocument b = new BasicDocument(text);
				double efficientFleschScore = b.getFleschScore();
			}
			
			long efficientETime = System.nanoTime();
			double efficientTime = (efficientETime - efficientSTime) / 1000000000.0 / trials;
